...
const ProductDetails = ({productDetails}) => {
  ...
  const [showOption, setShowOption] = useState("best")
  ...
  const toggleOptions = e => {
    e.preventDefault()
    showOption == "best" ? setShowOption("not best") : setShowOption("best")
  }
  ...
  return (
    <div>
      <div className="flex w-full justify-center">
        <button type="button" className="px-8 py-3 font-semibold rounded bg-gray-100 text-gray-800" onClick={toggleOptions}>Toggle Options</button>
      </div>
      {showOption == "not best" &&
        <div>
